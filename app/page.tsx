...
import MultiLocationWorkspace from './MultiLocationWorkspace';
...
export default function Home() {
  const [activeWorkspace, setActiveWorkspace] = useState<'single' | 'multi'>('single');
  ...
  return (
    <main className="app-shell">
      ...
      <nav className="workflow-tabs" aria-label="Tipo de análise" role="tablist">
        <button id="single-location-tab" type="button" role="tab" aria-controls="single-location-panel" aria-selected={activeWorkspace === 'single'} className={activeWorkspace === 'single' ? 'active' : ''} onClick={() => setActiveWorkspace('single')}><span>Local único</span><small>Observations e Plots</small></button>
        <button id="multi-location-tab" type="button" role="tab" aria-controls="multi-location-panel" aria-selected={activeWorkspace === 'multi'} className={activeWorkspace === 'multi' ? 'active' : ''} onClick={() => setActiveWorkspace('multi')}><span>Multi-location</span><small>Médias por genótipo</small></button>
      </nav>

      <div id="single-location-panel" className="workflow-panel" role="tabpanel" aria-labelledby="single-location-tab" hidden={activeWorkspace !== 'single'}>
      ...
      <div id="multi-location-panel" className="workflow-panel" role="tabpanel" aria-labelledby="multi-location-tab" hidden={activeWorkspace !== 'multi'}>
        <MultiLocationWorkspace />
      </div>

      <footer><span>Phenome OBS</span><p>Ferramenta local para organizar extrações de campo e médias multi-location.</p></footer>
    </main>
  );
}
